...
sql="select top 1 orders,Product_ID from Product_Info where ProductType_ID="&request("ProductType_ID")&" and orders<" & sorders & " order by orders desc"
set rs=conn.execute(sql)
if rs.eof then
	Response.Write "<script language=javascript>alert('该记录已移动到最下！');history.back()</script>"
	Response.End 
end if
